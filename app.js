...
function render(){
  ...
  const prompt=m.promptArt?`<div class="quiz-prompt" aria-hidden="true"><img src="${m.promptArt}" alt=""></div>`:"";
  let quiz=`<div class="quiz-copy">${prompt}<div class="question">${m.question}</div>`;
...
function answerFormula(){
  ...
  if(placed.some(id=>!id)){toast("Glisse toutes les étiquettes dans la formule.");return}
  locked=true;
